...
public class FactoryAfula extends Fragment {
    View myView;
    GridView gvNamesFactoryAfula;
    public Employee[] employeesFactoryAfula;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myView = inflater.inflate(R.layout.fragment_factory_afula, container, false);
        getActivity().setTitle("ייצור - עפולה");
        setHasOptionsMenu(true);

        int i=0;
        for (Employee employee : (((MainActivity) getActivity()).employees)) { //
            if (employee == null) {break;}
            if (employee.getDepartment() == 4) { // Checks number of employees in department to create array.
                i++;
            }
        }
        employeesFactoryAfula = new Employee[i];
        i=0;
        for (Employee employee : (((MainActivity) getActivity()).employees)) {
            if (employee == null) {break;}
            if (employee.getDepartment() == 4) { // Gets only the needed employees
                employeesFactoryAfula[i] = employee;
                i++;
            }
        }

        startTimer();
        gvNamesFactoryAfula = (GridView) myView.findViewById(R.id.gridviewNamesFactoryAfula);
        EmployeesAdapter employeeAdapter = new EmployeesAdapter(getActivity(), employeesFactoryAfula);
        gvNamesFactoryAfula.setAdapter(employeeAdapter);
        gvNamesFactoryAfula.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v,
                                    int position, long id) {
                Snackbar.make(v, "pressed image id: " +  + employeesFactoryAfula[position].getId(), Snackbar.LENGTH_LONG)
                        .setAction("Action", null).show();
                history_dialog(employeesFactoryAfula[position].getId());
            }
        });
        return myView;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        int m=0,l=0,o=0;
        for (Employee employee : employeesFactoryAfula) { //
            if (employee == null) {break;}
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_all:
                employeeAdapter = new EmployeesAdapter(getActivity(), employeesFactoryAfula);
                gvNamesFactoryAfula.setAdapter(employeeAdapter);
...
//                message = "run attendence thread";
                ...
                while (!recievedMsg.toLowerCase().contains("done")){
                    try {
                        ...
                        for (i = 0; i < employeesFactoryAfula.length; i++) {
                            if (employeesFactoryAfula[i].getId() == id) {
                                employeesFactoryAfula[i].setStatus(status);
                                break;
                            }
                        }
                        ((MainActivity) getActivity()).dos.writeUTF("recieved");

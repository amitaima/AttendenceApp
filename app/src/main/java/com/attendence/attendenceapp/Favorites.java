...
public class Favorites extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        employeesFavoritesTemp = new Employee[(((MainActivity) getActivity()).employees).length];
        int i = 0;
        int j = 0;
        for (Employee employee : (((MainActivity) getActivity()).employees)) {
            ...
            if (employee.getIsFavorite()) { // Gets only the needed employees
                employeesFavoritesTemp[i] = employee;
                i++;
            }
        }
        if (employeesFavoritesTemp[0] == null) {
            textViewEmpty.setVisibility(View.VISIBLE);
        } else {
            employeesFavorites = new Employee[i];
            for (j = 0; j < i; j++) {
                employeesFavorites[j] = employeesFavoritesTemp[j];
            }
            employeeAdapter = new EmployeesAdapter(getActivity(), employeesFavorites); //((MainActivity) getActivity()).employees
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_all:
                employeeAdapter = new EmployeesAdapter(getActivity(), employeesFavorites);
                gvNamesFavorites.setAdapter(employeeAdapter);
...
//                InputStream is;
                ...
                while (!recievedMsg.toLowerCase().contains("done")) {
                    try {
                        ...
                        for (i = 0; i < employeesFavorites.length; i++) {
                            if (employeesFavorites[i].getId() == id) {
                                employeesFavorites[i].setStatus(status);
                                break;
                            }
                        }
                        ((MainActivity) getActivity()).dos.writeUTF("recieved");

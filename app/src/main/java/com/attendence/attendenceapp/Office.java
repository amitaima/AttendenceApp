...
public class Office extends Fragment {
    ...
    View myView;
    GridView gvNamesOffice;
    public Employee[] employeesOffice;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myView = inflater.inflate(R.layout.fragment_office, container, false);
        getActivity().setTitle("משרד");
        ...
        for (Employee employee : (((MainActivity) getActivity()).employees)) { //
            ...
            if (employee.getDepartment() == 0) { // Checks number of employees in department to create array.
                i++;
            }
        }
        employeesOffice = new Employee[i];//(((MainActivity) getActivity()).employees).length
        i=0;
        for (Employee employee : (((MainActivity) getActivity()).employees)) { //
            if (employee == null) {break;}
            if (employee.getDepartment() == 0) { // Gets only the needed employees
                employeesOffice[i] = employee;
                i++;
            }
        }
        //new Thread(new statusThread()).start();
//        statusThread = new Thread(new alwaysRunThread());
//        statusThread.start();
        startTimer();
        gvNamesOffice = (GridView) myView.findViewById(R.id.gridviewNamesOffice);
        EmployeesAdapter employeeAdapter = new EmployeesAdapter(getActivity(), employeesOffice);
        gvNamesOffice.setAdapter(employeeAdapter);
        gvNamesOffice.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v,
                                    int position, long id) {
                Snackbar.make(v, "מספר עובד: " +  + employeesOffice[position].getId(), Snackbar.LENGTH_LONG)
                        .setAction("Action", null).show();
                employeePosition = position;
                history_dialog(employeesOffice[position].getId(),employeesOffice[position]);
            }
        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        int m=0,l=0,o=0,p=0;
        for (Employee employee : employeesOffice) { //
            if (employee == null) {break;}
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_all:
                employeeAdapter = new EmployeesAdapter(getActivity(), employeesOffice);
                gvNamesOffice.setAdapter(employeeAdapter);
    ...
    public void history_dialog(int id,Employee employee) {
        ...
        employeeDepartment.setText("מחלקה: משרד");
...
//                message = "run attendence thread";
                ...
                while (!recievedMsg.toLowerCase().contains("done")){
                    try {
                        ...
                        for (i = 0; i < employeesOffice.length; i++) {
                            if (employeesOffice[i].getId() == id) {
                                employeesOffice[i].setStatus(status);
                                break;
                            }
                        }
                        employeeAdapter = new EmployeesAdapter(getActivity(), employeesOffice);
    ...
    /*class statusThread implements Runnable {
        ...
        public void run() {
                try {
                    ...
                    String recievedMsg = "";
                    InputStream is;
                    dos = new DataOutputStream(((MainActivity) getActivity()).socket.getOutputStream());
                    message = "get attendence";
                    dos.writeUTF(message);
                    if (((MainActivity) getActivity()).socket.getInputStream() != null) {
                        recievedMsg = "";
                        is = ((MainActivity) getActivity()).socket.getInputStream();
                        byte[] buffer = new byte[12];
                        String[] splitedStr;
                        int id, onTime, inOrOut, i, j;
                        while (!recievedMsg.toLowerCase().contains("done")) {
                            try {
                                // Getting next message:
                                is = ((MainActivity) getActivity()).socket.getInputStream();
                                ...
                                for (i = 0; i < employeesOffice.length; i++) {
                                    if (employeesOffice[i].getId() == id) {
                                        if (inOrOut == 1) {
                                            employeesOffice[i].setStatus(onTime + 1);
                                        } else {
                                            employeesOffice[i].setStatus(0);
                                        }
                                        break;
                                    }
                                }
                        ...
                        is.close();
                    } else if (((MainActivity) getActivity()).socket.getInputStream() == null) {
                        BufferedReader in =
                                new BufferedReader(new InputStreamReader(((MainActivity) getActivity()).socket.getInputStream()));
                        message = in.toString();
                    }
                    employeeAdapter = new EmployeesAdapter(getActivity(), employeesOffice);
                    getActivity().runOnUiThread(new Runnable() {

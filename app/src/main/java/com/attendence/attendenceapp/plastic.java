...
public class plastic extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        for (Employee employee : (((MainActivity) getActivity()).employees)) { //
            ...
            if (employee.getDepartment() == 1) { // Checks number of employees in department to create array.
                i++;
            }
        }
        employeesPlastic = new Employee[i];
        i=0;
        for (Employee employee : (((MainActivity) getActivity()).employees)) {
            if (employee == null) {break;}
            if (employee.getDepartment() == 1) { // Gets only the needed employees
                employeesPlastic[i] = employee;
                i++;
            }
        }

        startTimer();
        gvNamesPlastic = (GridView) myView.findViewById(R.id.gridviewNamesPlastic);
        EmployeesAdapter employeeAdapter = new EmployeesAdapter(getActivity(), employeesPlastic);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_all:
                employeeAdapter = new EmployeesAdapter(getActivity(), employeesPlastic);
                gvNamesPlastic.setAdapter(employeeAdapter);
...
//                message = "run attendence thread";
                ...
                while (!recievedMsg.toLowerCase().contains("done")){
                    try {
                        ...
                        for (i = 0; i < employeesPlastic.length; i++) {
                            if (employeesPlastic[i].getId() == id) {
                                employeesPlastic[i].setStatus(status);
                                break;
                            }
                        }
                        ((MainActivity) getActivity()).dos.writeUTF("recieved");
